...
const { sendOrderConfirmation } = require('./utils/email'); // Resend — order confirmation emails
const { sendStatusUpdateEmail } = require('../utils/email'); // SendGrid — status update emails (separate service)
...
module.exports = async (req, res) => {
    ...
    async function verifySession(req, client) {
        ...
        try {
            const result = await client.query(`SELECT * FROM ${AUTH_SCHEMA}.verify_session_v3($1::TEXT)`, [sessionToken]);
    ...
    try {
        ...
        if (method === 'GET') {
            ...
            if (query.action === 'getReviews') {
                ...
                const result = await client.query(
                    'SELECT id, reviewer_name, rating, comment, created_at FROM reviews WHERE product_id = $1 AND is_approved = true ORDER BY created_at DESC LIMIT 50',
                    [productId]
                );

                // 2. Get Stats (Average & Count)
                const statResult = await client.query(
                    'SELECT COALESCE(AVG(rating), 0) as avg_rating, COUNT(*) as total_count FROM reviews WHERE product_id = $1 AND is_approved = true',
                    [productId]
                );
            ...
            // --- BLOG: Get Single Post by Slug (Public) ---
            ...
            // --- BLOG: Get All Posts including drafts (Admin) ---
        ...
        if (method === 'POST') {
            ...
            if (query.action === 'changeAdminPassword') {
                ...
                if (sessionToken) {
                    await client.query(`SELECT ${AUTH_SCHEMA}.delete_session($1::TEXT)`, [sessionToken]);
                }
