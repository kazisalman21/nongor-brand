    ...
    try {
        ...
        if (query.action === 'getOrderDetails') {
            ...
            try {
                ...
                await client.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS subtotal NUMERIC DEFAULT 0");
            } catch (err) {
                console.error('Migration failed (non-critical):', err.message);
            }

            // Ensure columns exist here too in case this is called first
            try {
                await client.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipping_fee NUMERIC DEFAULT 0");
                await client.query("ALTER TABLE orders ADD COLUMN IF NOT EXISTS subtotal NUMERIC DEFAULT 0");
            } catch (e) { }
